      ...
      <div className="flex-1 overflow-auto border border-slate-200 dark:border-slate-700 rounded-2xl bg-white dark:bg-slate-900/40 relative custom-scrollbar shadow-sm">
        <table className="w-full border-collapse text-left min-w-[600px]">
           <thead className="bg-white dark:bg-slate-900 sticky top-0 z-20 shadow-md">
               <tr>
                   <th className="p-4 w-48 text-slate-600 dark:text-slate-400 font-medium text-xs uppercase tracking-wider border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 sticky left-0 z-30">Specification</th>
                   {selectedTablets.map(tablet => (
                       <th key={tablet.id} className="p-4 w-64 min-w-[200px] border-b border-slate-200 dark:border-slate-700 relative group bg-white dark:bg-slate-900">
               ...
               {fields.map((field) => {
                   ...
                   return (
                       <tr key={field.key} className={`group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${!isIdentical ? 'bg-primary-50/30 dark:bg-primary-900/5' : ''}`}>
                           <td className="p-4 text-sm font-medium text-slate-600 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800/50 bg-slate-50/80 dark:bg-slate-900/20 sticky left-0 z-10 backdrop-blur-sm">
                               {field.label}
                           ...
                           {selectedTablets.map((tablet, index) => {
                               ...
                               return (
                                   <td 
                                     ...
                                     className={`p-4 text-sm border-r border-slate-100 dark:border-slate-800/50 last:border-r-0 relative ${
                                         isIdentical 
